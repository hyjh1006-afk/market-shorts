...
언어 규칙 (중요): 내레이션 네 필드는 100% 한국어 문장으로만 쓴다. 영어 뉴스 제목을 원문 그대로
넣지 말고 내용을 한국어로 옮겨 말한다. 기업·코인 이름은 한국어 표기를 쓴다
(Nvidia→엔비디아, Anthropic→앤트로픽, Bitcoin→비트코인). 티커나 영문 약어(ETF, AI 등)는 허용.
...
def _default_us_narration(us_stocks: list[dict]) -> str:
    ...
    from generators.text_gen import spoken_pct, top_movers
    top = top_movers(us_stocks, "ret_1d", 3)
    if not top:
        return ""
    line = ", ".join(f"{s['name']} {spoken_pct(s['ret_1d'])}" for s in top)
